...
class WGC2020_Process(base_process.Process):
    ...
    def __init__(self):
        ...
        self.step_range = [[0.1]]  # Each level must have a defined step, len(step_range) == number of MLMC levels
    ...
    def run(self, renew=False):
        ...
        for step_range in self.step_range:  # Length of step range determines number of levels
            ...
            # Schedule samples
            self.generate_jobs(sampler, n_samples=[3,3], renew=renew)
    ...
    def setup_config(self, step_range, clean):
        ...
        simulation_config = {
            ...
            'env': dict(flow123d=self.flow123d, gmsh=self.gmsh, gmsh_version=2),  # The Environment.
            ...
            'field_template': "!FieldFE {mesh_data_file: \"$INPUT_DIR$/%s\", field_name: %s, interpolation: identic_mesh}"
        }
    ...
    def set_environment_variables(self):
        ...
        if tail == 'storage':
            ...
            self.flow123d = "/home/paulie/docker_flow123d/flow123d/bin/fterm"
            self.gmsh = "/home/paulie/docker_flow123d/WGC2020-THM-MC/mlmc_random_frac/env/bin/gmsh"
